...
- 後續語言依照聊天歷史自然延續；使用者可在對話中明確指定其他語言。
- 翻譯或快捷提示個別設定的語言仍只在使用當次加入，不受首次請求限制。

---

## [2.22.25] — 2026-07-28
### Fixed — 對話內語言要求被重設
- 回覆語言現在只作為新對話的預設語言；使用者在對話中明確指定其他語言後，會持續沿用最近指定的語言，直到再次更改。
- 修正選擇「介面語言」後，第一輪依使用者要求改用英文、下一輪卻被重新切回介面語言的問題。
- 翻譯及快捷提示的單次語言設定仍只影響當次請求，不會意外改變整個對話的語言偏好。

---

## [2.22.24] — 2026-07-28
### Fixed — 首次使用尚未設定模型
- 語言選項不依賴 AI 模型，首次使用時可正常儲存，不會翻譯使用者輸入或呼叫模型。
- 尚未設定可用模型時，傳送訊息或點擊快捷提示會在送出前攔截，保留輸入內容並引導使用者前往設定，不再回退至不存在的預設模型或顯示 API 錯誤。

---

## [2.22.23] — 2026-07-28
### Improved — 回覆語言選項語意與動態說明
- 將容易誤解的「自動」改為「訊息語言（預設）」，明確表示這是沿用原本依照最新訊息語言回答的行為。
- AI 回覆語言下方的說明會依目前選項即時更新，分別說明訊息語言、介面語言或固定語言的實際回答方式。

---

## [2.22.22] — 2026-07-28
### Improved — 語言下拉選項精簡
- 精簡語言下拉選項為「自動」、「介面語言」、「AI 回覆設定」及「輸入語言」等短標籤，避免窄選單截斷文字。
- 將各選項的完整行為保留在欄位說明中，兼顧可讀性與操作清晰度。

---

## [2.22.21] — 2026-07-28
### Improved — AI 回覆語言說明
- 將「自動識別」改為更明確的「跟隨你的訊息語言」，並說明 AI 會直接以所選語言回答，不會先翻譯使用者訊息。
- 補充系統提示詞、一般快捷提示與翻譯快捷提示的語言繼承及覆寫關係，讓使用者能預期最終回答語言。

---

## [2.22.20] — 2026-07-28
### Fixed — 快捷提示遵循 AI 回覆語言
- 修正「摘要」、「潤飾文字」與「繼續寫作」預設強制跟隨英文快捷提示，導致全域選擇日文等語言後仍以英文回答的問題。
- 非翻譯快捷提示現在預設繼承「AI 回覆語言」；既有內建快捷提示會自動遷移，同時保留使用者編輯過的標題與提示內容。

---

## [2.22.19] — 2026-07-28
### Added — AI 回覆語言
- 在「語言設定」下方新增獨立的「AI 回覆語言」，可自動識別、跟隨介面，或指定繁中、簡中、英文、日文及韓文。
- 系統提示詞可個別覆寫全域回覆語言；快捷提示可指定輸出語言，翻譯快捷提示則使用獨立的目標語言語意。
- 語言規則在送出時動態加入，不翻譯或覆蓋使用者原有提示詞；使用者在當次訊息明確指定語言時仍具有最高優先級。
- 內建快捷提示標題會依介面顯示繁中、簡中或英文，使用者自建標題維持原文。

---

## [2.22.18] — 2026-07-28
### Fixed — 設定備份分隔線留白
- 增加「設定備份」頂部分隔線與標題之間的垂直留白，避免標題緊貼分隔線。

---

## [2.22.17] — 2026-07-28
### Improved — 提示詞配置密度
- 縮小「系統提示詞」與「快捷提示」配置卡片的高度、內邊距、間距與圓角，在保留操作按鈕點擊範圍的同時顯示更多項目。

---

## [2.22.16] — 2026-07-28
### Improved — Web Search 準確度與安全性
- 修正聯網搜尋開關語義：開啟時按訊息意圖智能搜尋；關閉時仍支援明確要求「搜尋／上網查」的訊息。
- 搜尋資料只注入模型上下文一次，保留原始系統提示，減少重複 token 與規則覆蓋。
- 網頁資料標示為不可信證據，要求模型不得執行或遵從頁面內的指令，降低間接提示注入風險。
- 訊息中的網址成功讀取後不再無條件二次搜尋；只有明確要求最新資訊、比較或額外搜尋時才同時搜尋。
- 搜尋結果新增 URL 正規化去重與相關性排序；網頁正文改為抽取與問題相關的段落，不再只截取頁面開頭。
- Brave Search 會使用介面語言、時效條件與額外摘要；Tavily 會依新聞／金融意圖選擇搜尋主題。

---

## [2.22.15] — 2026-07-28
### Fixed — Smart Capture 動態頁面完整性
- 修正定價頁、產品頁與 SPA 使用雜湊 class 或將文字放在 `div` / `span` 時，Smart Capture 可能漏掉方案、價格與功能內容的問題。
- 可見文字擷取現在會與 `main.innerText` 計算覆蓋率；覆蓋率不足時自動回退到完整可見正文。
- 偵測未選取的分頁、收合區域、iframe / canvas、Shadow DOM、虛擬內容與字符數截斷，並在引用卡片明示「可能不完整」。
- 可能不完整的引用會同時加入模型側警告，要求 AI 不得將內容視為完整或窮盡性資料。
- 新安裝或尚未設定時，頁面內容字符數上限由 20,000 提高至 50,000；既有使用者已保存的數值維持不變。
- Full Page 模式不再套用字符數上限；設定頁會停用該輸入並說明完整模式會傳送全部已載入內容。
- 擷取模式簡化為 Smart Capture、Limited Capture 與 Full Page；Smart 與 Full 不設字符上限，只有 Limited 套用字符數設定，舊模式會自動遷移至 Smart Capture。
- 修正 Chrome 視窗高度縮小時，歡迎頁建議列表與聊天區同時出現兩條垂直捲軸的問題。

---

## [2.22.14] — 2026-06-26
### Changed — 聯網搜尋預設抓取網頁正文
- 「簡易搜尋模式」預設改為**關閉**：聯網搜尋現在預設會額外訪問前 3 筆搜尋結果的頁面、取得完整正文，回答更精確（仍可在設定頁開回簡易模式只用摘要）。
- 前 3 筆網頁正文改為**並行抓取**，避免串行等待造成過長延遲。

---

## [2.22.13] — 2026-06-26
### Fixed — 聯網搜尋打開後不搜尋
- 修正手動打開「聯網搜尋」開關後，仍被 `shouldSearch` 啟發式否決而實際沒有搜尋、回答中沒有整合搜尋結果的問題。
- 現在手動打開開關 = 本回合一定搜尋；`shouldSearch` 啟發式僅用於開關關閉時、訊息明確要求搜尋（如「幫我搜一下」）的自動觸發。

### Fixed — 長對話崩潰（記憶體 / 串流渲染）
- 發送訊息給 API 時不再無上限 hydrate 全部歷史圖片 base64，只保留最近 2 輪的圖片資料，較舊訊息僅保留文字，避免長對話 + 多圖時記憶體暴漲導致側邊欄崩潰。
- 串流輸出超長內容（>80,000 字）時，過程中改用純文字顯示，避免每次節流更新都重新解析全文 Markdown（O(n²)），生成結束後恢復完整 Markdown 渲染。
- `persistSessions` 寫入 `chrome.storage.local` 加上配額超限的清理重試與錯誤記錄，避免長對話寫入失敗時靜默丟失資料。
- `finalizeStreamingMessage` 會清除殘留的串流節流定時器，避免完成後舊定時器再覆蓋一次渲染。
