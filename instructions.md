...
| 使用者文件 | `site/` HTML → GitHub Pages `https://tomideas.github.io/momo-assist/` |
...
├── site/                  使用者說明書 HTML（對外，唯一來源）
├── docs/                  內部筆記（UI-SPEC、速查表等，本地）
...
3. 讀 `CHANGELOG.md` 頂部 1–2 個版本，了解最近變更。
4. 若任務涉及 UI，讀 `docs/UI-SPEC.md` 的相關 token 區塊。
...
### E. 修改 docs
改 `site/` HTML → 同步 README 連結 → push → 確認 Pages workflow
