...
├── temp/                  ⚠️ 暫存工作區，不是正式來源
│   ├── momo-assist-repo/  git clone，rsync 後 push 用（push 完可刪再 clone）
│   ├── research/          競品研究，不要複製進正式功能
│   └── tomideas-site/     已棄用跳轉 repo，待刪
├── instructions.md        本檔（給 AI 協作）
├── README.md / README.zh-TW.md
└── .github/workflows/deploy-docs.yml
```

**不要改動或依賴：** `temp/research/`、`temp/tomideas-site/`（已棄用）、`keys/`（敏感）。
...
**重要：** `workspace/docs/` 和 `workspace/sider/` 是唯一修改來源。  
`temp/momo-assist-repo/` 只是 rsync 目標，不要直接在裡面改檔案。

```bash
SOURCE="<workspace>"
REPO="$SOURCE/temp/momo-assist-repo"

# Step 1: rsync 工作區 → git repo
rsync -av --delete --exclude '.git/' --filter='- **/._*' --filter='- ._*' \
  "$SOURCE/sider/" "$REPO/sider/"
rsync -av --delete --exclude '.git/' --filter='- **/._*' --filter='- ._*' \
  "$SOURCE/docs/" "$REPO/docs/"
rsync -av --delete --exclude '.git/' --filter='- **/._*' --filter='- ._*' \
  "$SOURCE/.github/" "$REPO/.github/"
cp "$SOURCE/README.md" "$SOURCE/README.zh-TW.md" "$REPO/"
...
# Step 3: Release zip
~/.cursor/skills/github-release-sync/scripts/github-release.sh "$REPO"
```
