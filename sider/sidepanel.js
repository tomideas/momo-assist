...
let lastStandardSessionId = null;
...
const OPENCLAW_LOCAL_SESSION_ID = 'momo-openclaw-main';
...
function isSelectedOpenClawProvider(providerConfigs){
  const providerId = getSelectedModelProviderId();
  return providerId === 'openclaw' || !!providerConfigs?.[providerId]?.isOpenClaw;
}
...
function isOpenClawLocalSession(session){
  return !!session && (session.id === OPENCLAW_LOCAL_SESSION_ID || session._agentProvider === 'openclaw');
}

function isStandardLocalSession(session){
  return !!session
    && !isHermesLocalSession(session)
    && !isOpenClawLocalSession(session)
    && !session.messages?.some(m => m._fromOpenClawHistory);
}
...
function ensureOpenClawLocalSession({ render = false, providerConfigs = null } = {}){
  if(!isSelectedOpenClawProvider(providerConfigs)) return false;
  if(!sessionsLoaded) return false;
  let changed = false;
  let session = sessions.find(s => s.id === OPENCLAW_LOCAL_SESSION_ID || s._agentProvider === 'openclaw');
  if(!session){
    session = {
      id: OPENCLAW_LOCAL_SESSION_ID,
      title: 'OpenClaw',
      createdAt: Date.now(),
      messages: [],
      _agentProvider: 'openclaw'
    };
    sessions.unshift(session);
    changed = true;
  }else{
    if(session.id !== OPENCLAW_LOCAL_SESSION_ID){ session.id = OPENCLAW_LOCAL_SESSION_ID; changed = true; }
    if(!session.title){ session.title = 'OpenClaw'; changed = true; }
    if(session._agentProvider !== 'openclaw'){ session._agentProvider = 'openclaw'; changed = true; }
  }
  if(currentSessionId !== session.id){
    currentSessionId = session.id;
    changed = true;
...
function restoreStandardSession({ render = false } = {}){
  if(!sessionsLoaded) return false;
  let session = lastStandardSessionId
    ? sessions.find(s => s.id === lastStandardSessionId && isStandardLocalSession(s))
    : null;
  if(!session){
    session = sessions.find(s => isStandardLocalSession(s));
  }
  if(!session){
    createNewSession(true);
  }else{
    currentSessionId = session.id;
    lastStandardSessionId = session.id;
    persistSessions();
    syncSystemMessage();
  }
  if(render){
...
function ensureFreshStandardSession(providerConfigs, { render = false, force = false } = {}){
  ...
  const shouldReset = force
    ...
    || isOpenClawLocalSession(cur)
    || !!cur?.messages?.some(m => m._fromOpenClawHistory);
  if(!shouldReset) return false;

  restoreStandardSession({ render });
  console.log('[SP] Standard provider selected; restored standard session:', providerId);
  return true;
}
...
async function handleAgentProviderModelSwitch(nextUid, previousUid){
  ...
  const { providerConfigs } = await chrome.storage.local.get('providerConfigs');
  const nextIsAgentProvider = isAgentProviderId(nextProvider, providerConfigs);
  const previousWasAgentProvider = isAgentProviderId(previousProvider, providerConfigs);
  if(nextIsAgentProvider && !previousWasAgentProvider && isStandardLocalSession(getCurrentSession())){
    lastStandardSessionId = currentSessionId;
  }

  if(!nextIsAgentProvider){
    if(previousWasAgentProvider){
  ...
  if(nextProvider === 'openclaw' || providerConfigs?.[nextProvider]?.isOpenClaw){
    ensureOpenClawLocalSession({ render: true, providerConfigs });
    console.log('[SP] OpenClaw model selected, using isolated local session:', OPENCLAW_LOCAL_SESSION_ID);
    uploadedImages = [];
    renderImagePreviews();
    resetComposerIfEmpty();
    return;
...
async function updateOpenClawPromptVisibility(){
  try{
    ...
    if(isOpenClaw){
      ...
      // Standard providers share the current Momo conversation. Session changes
      // are handled only when explicitly leaving an agent provider.
    }
  }catch(e){
...
async function loadAndShowOpenClawHistory(){
  ...
  try{
    ...
    ensureOpenClawLocalSession({ providerConfigs });
    if(isStale()) return;
    const session = getCurrentSession();
    if(!session) return;
...
function persistSessions(){
  ...
  if(isStandardLocalSession(currentSession)){
    lastStandardSessionId = currentSession.id;
  }
